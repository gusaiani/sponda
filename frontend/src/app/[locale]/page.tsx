...
function SeoArticle({ locale }: { locale: string }) {
  const ARTICLES: Record<string, { title: string; sections: { heading: string; text: string }[] }> = {
    pt: {
      title: "Análise fundamentalista de ações brasileiras ajustada pela inflação",
      sections: [
    ...
    en: {
      title: "Inflation-adjusted fundamental analysis of Brazilian stocks",
      sections: [
    ...
    es: {
      title: "Análisis fundamental de acciones brasileñas ajustado por inflación",
      sections: [
    ...
    fr: {
      title: "Analyse fondamentale d'actions brésiliennes ajustée de l'inflation",
      sections: [
    ...
    de: {
      title: "Inflationsbereinigte Fundamentalanalyse brasilianischer Aktien",
      sections: [
  ...
  return (
    <article className="homepage-explainer" style={SEO_STYLE}>
      ...
      <p>Sponda {locale === "pt" ? "é uma plataforma de análise fundamentalista para investidores em valor." : locale === "es" ? "es una plataforma de análisis fundamental para inversores en valor." : locale === "zh" ? "是一个面向价值投资者的基本面分析平台。" : locale === "fr" ? "est une plateforme d'analyse fondamentale pour investisseurs value." : locale === "de" ? "ist eine Fundamentalanalyse-Plattform für Value-Investoren." : "is a fundamental analysis platform for value investors."}</p>
