...
import { localizeLabel, formatNumber, formatLargeNumber, currencySymbol, currencyCode, formatQuarterLabel } from "../utils/format";
...
/* ── Chart series builder (shared with the comparison overlay) ── */

/** The windowed company fields the chart series math reads. A subset of
 * QuoteData / QuoteResult, so either can be passed directly. */
...
/**
 * Build every indicator's chart series from one company's windowed data. Pure
 * and exported so the comparison overlay reuses the exact same series math for
 * each added company instead of re-deriving it divergently.
 */
export function buildChartData(
  data: ChartSourceData,
  priceHistory: { date: string; adjustedClose: number }[] | undefined,
  fundamentals: import("../hooks/useFundamentals").FundamentalsYear[] | undefined,
  quarterlyRatios: import("../hooks/useFundamentals").QuarterlyBalanceRatio[] | undefined,
  years: number,
...
export function CompanyMetricsCard({ data, years, maxYears, onYearsChange, sector, fundamentals, quarterlyRatios, priceHistory }: CompanyMetricsCardProps) {
  ...
  const chartData = useMemo(
    () => buildChartData(data, priceHistory, fundamentals, quarterlyRatios, years),
    [data, priceHistory, fundamentals, quarterlyRatios, years],
  );
  ...
  /* Indicator labels resolved every render, so the expanded-chart modal
   * title tracks the term slider (PE5 → PE13) instead of freezing the
   * label captured when the chart was expanded. */
  const metricLabelById: Record<string, string> = {
    [METRIC_IDS.currentPrice]: t("metrics.current_price"),
    [METRIC_IDS.marketCap]: t("metrics.market_cap"),
    [METRIC_IDS.debtToEquity]: t("metrics.gross_debt_equity"),
    [METRIC_IDS.debtExLease]: t("metrics.debt_ex_lease_equity"),
    [METRIC_IDS.liabToEquity]: t("metrics.liab_equity"),
    [METRIC_IDS.debtToEarnings]: t("metrics.gross_debt_earnings"),
    [METRIC_IDS.debtToFCF]: t("metrics.gross_debt_fcf"),
    [METRIC_IDS.currentRatio]: t("metrics.current_ratio"),
    [METRIC_IDS.pe10]: pl10Label,
    [METRIC_IDS.peg]: "PEG",
    [METRIC_IDS.cagrEarnings]: t("metrics.cagr_earnings"),
    [METRIC_IDS.pfcf10]: pfcl10Label,
    [METRIC_IDS.pfcfg]: t("metrics.pfcfg_label"),
    [METRIC_IDS.cagrFCF]: t("metrics.cagr_fcf"),
  };

  const renderExpandButton = (metricId: string) => {
    if (!showGraphs) return null;
    const series = chartData[metricId];
    if (!series || series.length < 2) return null;
    return <ExpandButton label={t("metrics.expand_chart")} onClick={() => setExpandedChartMetricId(metricId)} />;
  };
    ...
    <article className="pe10-card" aria-label={`${data.name} (${data.ticker})`}>
      ...
      <div className="metrics-row">
        <div id={METRIC_IDS.currentPrice} {...metricBlockProps(METRIC_IDS.currentPrice)}>
          ...
          {renderExpandButton(METRIC_IDS.currentPrice)}
        ...
        <div id={METRIC_IDS.marketCap} {...metricBlockProps(METRIC_IDS.marketCap)}>
          ...
          {renderExpandButton(METRIC_IDS.marketCap)}
          <div className="metric-value-container">
      ...
      <div className="card-section">
        ...
        <div className="metrics-row leverage-row-6col">
          <div id={METRIC_IDS.debtToEquity} {...metricBlockProps(METRIC_IDS.debtToEquity)}>
            ...
            {renderExpandButton(METRIC_IDS.debtToEquity)}
            <div className="metric-value-container">
          ...
          {data.debtExLeaseToEquity !== null && (
            <div id={METRIC_IDS.debtExLease} {...metricBlockProps(METRIC_IDS.debtExLease)}>
              ...
              {renderExpandButton(METRIC_IDS.debtExLease)}
              <div className="metric-value-container">
          ...
          <div id={METRIC_IDS.liabToEquity} {...metricBlockProps(METRIC_IDS.liabToEquity)}>
            ...
            {renderExpandButton(METRIC_IDS.liabToEquity)}
            <div className="metric-value-container">
          ...
          <div id={METRIC_IDS.debtToEarnings} {...metricBlockProps(METRIC_IDS.debtToEarnings)}>
            ...
            {renderExpandButton(METRIC_IDS.debtToEarnings)}
            <div className="metric-value-container">
          ...
          <div id={METRIC_IDS.debtToFCF} {...metricBlockProps(METRIC_IDS.debtToFCF)}>
            ...
            {renderExpandButton(METRIC_IDS.debtToFCF)}
            <div className="metric-value-container">
          ...
          <div id={METRIC_IDS.currentRatio} {...metricBlockProps(METRIC_IDS.currentRatio)}>
            ...
            {renderExpandButton(METRIC_IDS.currentRatio)}
            <div className="metric-value-container">
      ...
      <div className="card-section">
        ...
        <div className="metrics-row valuation-row-6col">
          <div id={METRIC_IDS.pe10} {...metricBlockProps(METRIC_IDS.pe10)}>
            ...
            {renderExpandButton(METRIC_IDS.pe10)}
            <div className="metric-value-container">
          ...
          <div id={METRIC_IDS.peg} {...metricBlockProps(METRIC_IDS.peg)}>
            ...
            {renderExpandButton(METRIC_IDS.peg)}
            <div className="metric-value-container">
          ...
          <div id={METRIC_IDS.cagrEarnings} {...metricBlockProps(METRIC_IDS.cagrEarnings)}>
            ...
            {renderExpandButton(METRIC_IDS.cagrEarnings)}
            <div className="metric-value-container">
          ...
          <div id={METRIC_IDS.pfcf10} {...metricBlockProps(METRIC_IDS.pfcf10)}>
            ...
            {renderExpandButton(METRIC_IDS.pfcf10)}
            <div className="metric-value-container">
          ...
          <div id={METRIC_IDS.pfcfg} {...metricBlockProps(METRIC_IDS.pfcfg)}>
            ...
            {renderExpandButton(METRIC_IDS.pfcfg)}
            <div className="metric-value-container">
          ...
          <div id={METRIC_IDS.cagrFCF} {...metricBlockProps(METRIC_IDS.cagrFCF)}>
            ...
            {renderExpandButton(METRIC_IDS.cagrFCF)}
            <div className="metric-value-container">
      ...
      {expandedChartMetricId && chartData[expandedChartMetricId] && (
        <IndicatorChartModal
          indicatorLabel={metricLabelById[expandedChartMetricId]}
          metricId={expandedChartMetricId}
          primary={{
            ticker: data.ticker,
            name: data.name,
            currency: currencyCode(data.ticker),
            points: chartData[expandedChartMetricId],
          }}
          formatValue={chartValueFormatters[expandedChartMetricId]}
          years={years}
          maxYears={maxYears}
          onYearsChange={onYearsChange}
          onClose={() => setExpandedChartMetricId(null)}
        />
      )}
    </article>
  );
