...
- Over-cap requests get `429` with `{"code": "lookup_limit", ...}` and
  ...
  burned.
- **Attribution lives in one place, because getting it wrong is silent.**
  The anonymous scope filters on `ip_hash` alone, so a `LookupLog` row
  written with only a `session_key` is invisible to the cap: it is stored,
  it is never counted, and nothing fails. `BatchQuotesView` kept its own
  copy of the attribution logic and wrote exactly those rows, which left
  `POST /api/quotes/batch/` able to take a hundred symbols at a time
  without ever tripping the limit the single-company endpoints enforce.
  Nothing in the UI reached it that way — the home page only batches a
  signed-in visitor's favourites and saved lists, and a signed-in caller
  is scoped by user — so the hole was the endpoint being POSTed to
  directly. It now shares the mixin's `record_lookups`, which writes the
  whole batch in one INSERT (Sentry had also filed the per-ticker
  `INSERT` as an N+1, the only genuine database one of the set).
- The batch endpoint *records* lookups but does not yet *enforce* the cap
  before doing the work; `post` never calls `enforce_lookup_quota`. Its
  lookups now count against later single-company requests, which is what
  closes the enumeration hole in practice, but gating the batch itself
  would need a rule for partial batches (refuse the whole request, or
  serve the tickers that fit) and is deliberately left open.
...
X (Twitter) rendered every sponda.capital card with the correct title and description and no image, for months, across three rounds of fixes. Everything verifiable from the outside was clean: tags in the server-rendered HTML, HTTP 200 in ~210 ms, `image/jpeg`, baseline 1200×630, ~57 KB, absolute HTTPS URL, no Cloudflare rule touching images, no security action on any request from X's IP ranges (`199.16.156.0/22`, `192.133.76.0/22`). Bluesky, Discord, Facebook and WhatsApp all fetched and ingested the same image.

What Cloudflare's request analytics showed (GraphQL `httpRequestsAdaptiveGroups`, filtered on `userAgent_like: "%Twitterbot%"`, one day per query on the free plan; origin logs cannot show this because Cloudflare answers cached images itself):

1. For the static JPEG under `/images/`, X's IPs sent **only HEAD requests**, roughly twenty a day, and **never a GET** after launch day. Cause: `robots.txt` had `Disallow: /*/spond` for the social pages, and under RFC 9309 wildcard matching that also covers `/images/sponda-og-en-v2.jpg`. X checks `robots.txt` before every image download (the pairs of `robots.txt` + image requests in the logs are that), HEADs the URL, and obeys the rule for the GET. The `-v2` rename could never have helped. The social rules now end in `/`, and `frontend/src/lib/robots-rules.test.ts` runs our own file against every OG image path with a small RFC 9309 matcher.
2. For images under `/og/` (per-company PNGs, then the site card once it moved there), X's IPs sent **full GETs**, 35 to 58 KB each, four times per share from four addresses, and still rendered nothing. The one header those responses shared, and the static file never had, was Next's `Vary: rsc, next-router-state-tree, next-router-prefetch, next-router-segment-prefetch`, stamped on every app-router response. It is meaningless on an image. `nginx/sponda.capital.conf` now strips it on `location /og/` (`proxy_hide_header Vary`), pinned by `backend/tests/test_nginx_og_images.py`.

Along the way every OG image also gained a `Content-Length` and lost its validators (`ETag`, `Last-Modified`, `Accept-Ranges`) via `buildOgImageResponse()`. Neither was the root cause, but both remove a way for the next bad ingest to become permanent: a crawler that can revalidate a bad copy cheaply never downloads it again.

Two habits that would have shortened this: judge crawler access from the crawler's real IPs in Cloudflare analytics, never from `curl -A Twitterbot/1.0`; and read `robots.txt` with wildcard semantics in mind before trusting `Allow: /`. The same spoofed-user-agent trap cost four months of Googlebot indexing; see "Cloudflare cache purge" above for that story.

X caches a card per page URL for about seven days, so an already-shared link keeps its old imageless card. To verify a fix, share a URL X has not seen before (append a throwaway query string, e.g. `?v=4`), or paste the link into the composer and check the preview, which fetches fresh. `https://cdn.syndication.twimg.com/tweet-result?id=<tweet id>&token=x` returns the card X stored for a posted tweet, image bindings included, without logging in.

**Testing a card locally.** `npm run build && DJANGO_API_URL=https://sponda.capital npx next start -p 3199`, then open `http://localhost:3199/og/pt/VULC3.png` and `http://localhost:3199/og/site/en.jpg`. Pointing `DJANGO_API_URL` at production is the quickest way to render against real numbers. `curl -I` either URL and confirm `content-length` is present and `etag` and `vary` are not (the `vary` strip is nginx's, so it only shows in production).
